...
{
	global $con;
	$res = mysqli_query($con, "select dish.dish,dish_details.price from dish_details,dish where dish_details.id='$id' and dish.id=dish_details.dish_id");
	$row = mysqli_fetch_assoc($res);
	return $row;
}
...
{
	global $con;
	$sql = "delete from order_master where id='$oid'";
